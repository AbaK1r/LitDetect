...
from litdetect.callbacks import DetectionMetricsCallback, PicRecordCallback, DetectionMetrics3DCallback
...
def main():
    ...
    output_root = Path(tensorboard_logger.log_dir)
    (output_root / 'ckpts').mkdir(parents=True, exist_ok=True)
    for _version in _versions:
        ...
        call_back = [
            DetectionMetricsCallback(
                ...
                iou_threshold=0.5,
                save_xlsx_dir=output_root / 'result.xlsx',
                save_sheet_name=save_sheet_name
            ),
            PicRecordCallback(args.data.class_name, 0.25, output_root / save_sheet_name),
            DetectionMetrics3DCallback(
                args.data.class_name, data_csv_dir=Path(args.data.image_root).parent / 'val_tumor_mapping.csv',
