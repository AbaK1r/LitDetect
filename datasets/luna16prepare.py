...
LUNA16_OUTPUT_PATH = Path('/data/4t_hdd/DataSets/xjlDataset/Luna16results2')
YOLO_OUTPUT_PATH = LUNA16_OUTPUT_PATH / 'yolo_dataset'
LUNA16_OUTPUT_PATH.mkdir(parents=True, exist_ok=True)
...
def process_case(id, annos, filelist, output_path, split):
    ...
    try:
        ...
        for z, labels  in buffer.items():
            ...
            slice_img = exposure.rescale_intensity(slice_img, out_range=(0, 255)).astype(np.uint8)
...
def create_yolo_dataset():
    ...
    for split in ['train', 'val']:
        (YOLO_OUTPUT_PATH / split / 'images').mkdir(parents=True, exist_ok=True)
        (YOLO_OUTPUT_PATH / split / 'labels').mkdir(parents=True, exist_ok=True)
